...
export async function POST(req: Request) {
  try {
    ...
    if (file.size > 5 * 1024 * 1024) {  // Exemplo: máximo 5MB
      return NextResponse.json({ success: false, message: "Arquivo muito grande (máx. 5MB)." }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, message: "Imagem atualizada com sucesso." });
