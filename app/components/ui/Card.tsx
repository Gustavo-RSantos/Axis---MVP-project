...
export default function Card({
  ...
  function colorHandler(color : string){
   ...
   const TextBlock = (
        ...
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${colorHandler(color).title}`}>
            {iconStyle()}
            <span>{title}</span>
        </div>
        ...
        <ul className="space-y-4">
            {[contentList_1, contentList_2, contentList_3].map((item, index) => (
                <motion.li key={index} className="flex items-start gap-3 text-slate-700">
                    <CheckCircle2 className={`w-5 h-5 mt-0.5 shrink-0 ${colorHandler(color).circle}`} />
                    <span>{item}</span>
                </motion.li>
            ))}
        </ul>
